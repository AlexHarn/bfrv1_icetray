...
namespace I3GENIE_Helper
{
     ...
     * \param mcTree the tree to be filled
     * \paam weightDict the weighting information dictionary to be filled
     * \param resultDict the output of GENIE
     * \param positionShift a vector by which to shift particle positions when adding them to the MCTree
     * \return whether the conversion succeeded
     */
//These things are defined as pointers in I3GENIEResultDictToMCTree to make them nice to output to an i3 file
    bool ConvertToMCTree(I3MCTreePtr mcTree,  I3MapStringDoublePtr weightDict, I3GENIEResultDictConstPtr resultDict, const I3Position& positionShift);

    template <typename T>  
    const T &GetFromVariantMap(const std::string &name, const I3GENIEResultDict &dict);

    I3Position extractPosition(const I3GENIEResultDictConstPtr& resultDict, const I3Position& positionShift);
};
