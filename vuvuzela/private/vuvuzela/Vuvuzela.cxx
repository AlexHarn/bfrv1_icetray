...
Vuvuzela::Vuvuzela(const I3Context& ctx) : I3Module(ctx)
{
  ...
  goodOMTypes_ = {I3OMGeo::IceCube, I3OMGeo::PDOM, I3OMGeo::DEgg, I3OMGeo::mDOM};
  AddParameter("OMTypes", "Generate noise for these types of DOMs", goodOMTypes_);
 ...
 * OMTypes Simulate noise in DOMs of these types.
...
void Vuvuzela::Configure()
{
  ...
  GetParameter("OMTypes", goodOMTypes_);
...
void Vuvuzela::DAQ(I3FramePtr frame)
{
  ...
  I3ParticleIDMapConstPtr inputHitInfo;
  I3ParticleIDMapPtr outputHitInfo;
  ...
  if ( inputHitSeriesMapName_.size() ) {
    if (! frame->Has(inputHitSeriesMapName_)) {
      ...
      inputHitInfo = frame->Get<I3ParticleIDMapConstPtr>(inputHitSeriesMapName_+"ParticleIDMap");
      if (inputHitInfo) {
        outputHitInfo.reset(new I3ParticleIDMap(*inputHitInfo));
      }
    }
  }
  // Noise-Only mode
  ...
  I3MCPESeriesMapConstPtr outputHitMap = AddHitMaps(inputHitMap, noiseMap, outputHitInfo);
  ...
  if (outputHitSeriesMapName_.size() == 0) {
    ...
    if (inputHitInfo) {
      frame->Delete(inputHitSeriesMapName_+"ParticleIDMap");
      frame->Put(inputHitSeriesMapName_+"ParticleIDMap",outputHitInfo);
    }
  }
  // otherwise, use the new name
    ...
    if (inputHitInfo) {
      frame->Put(outputHitSeriesMapName_+"ParticleIDMap",outputHitInfo);
    }
  }
			   ...
			   const I3Calibration& calibration)
{
  ...
  for( geoIter  = geometry.omgeo.begin(); 
    ...
    log_debug("String: %i, OM:%i, PMT: %i, OMType: %i",
	     dom.GetString(), dom.GetOM(), dom.GetPMT(), geoIter->second.omtype);

    //only go for DOMs of the appropriate type
    bool keep = false;
    BOOST_FOREACH(int omtype, goodOMTypes_){
      keep = keep || (geoIter->second.omtype == omtype);
    }
    if( !keep ) continue;
...
I3MCPESeriesMapConstPtr Vuvuzela::GetNoiseHits(const I3Calibration& calibration,
  ...
  for(domIter = goodDOMs.begin(); domIter != goodDOMs.end(); ++domIter){
    ...
    std::set<double>& bufferHits = bufferMap[dom];
