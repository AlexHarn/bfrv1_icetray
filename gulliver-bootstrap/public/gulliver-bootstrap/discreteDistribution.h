...
bool compareFirst(PairType p1, PairType p2){ return(p1.first<p2.first); }
...
private:
    ...
    tableType table;
	static bool compProb(double x, const std::pair<double, unsigned int>& p){ return(x<p.first); }
public:
	...
	unsigned int operator()(random_adapter rand){
		...
		return(std::upper_bound(table.begin(),table.end(),x,&workaround_discrete_distribution::compProb)->second);
	}
};
