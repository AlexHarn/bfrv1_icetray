...
MillipedeLikelihood::GetMultiplicity()
{
	...
	// Count the number of valid bins with nonzero data. All zero bins can
	// be safely lumped together, since partitioning bins with zero
	// data does not change the Poisson likelihood.
	for (MillipedeDOMCacheMap::const_iterator i = domCache_.begin();
	    i != domCache_.end(); i++) {
		for (int j = 0; j < i->second.nbins; j++)
			if (i->second.charges[j] > 0 && i->second.valid[j])
				multiplicity++;
		nbins += i->second.valid.count();
	}

	// If multiplicity == nbins, then all bins were filled. Otherwise, add
	// 1 for the consolidated zero bin
	return (multiplicity == nbins) ? multiplicity : multiplicity+1;
}
