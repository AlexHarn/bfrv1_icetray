...
void G4Delaunay::CreateSurfaceTriangles(G4TessellatedSolid* solid)
{
  ...
  for (size_t i = 0; i < pointList_.size(); ++i) {
    const G4ThreeVector& point = pointList_.at(i);
    pts.push_back(Vector2<double>(point.x(), point.y(), i));
  }

  Delaunay<double> triangulation;
  std::vector<Triangle<double> > triangles = triangulation.triangulate(pts);

  log_trace("Found %zu Delaunay triangles:", triangles.size());
  for(unsigned int i=0; i<triangles.size(); ++i)
  {
    log_trace("%4d: (%3zu, %3zu, %3zu)", i, triangles[i].p1.id, triangles[i].p2.id, triangles[i].p3.id);
    G4TriangularFacet *facet = new G4TriangularFacet(pointList_[triangles[i].p1.id],
                                                     pointList_[triangles[i].p2.id],
                                                     pointList_[triangles[i].p3.id],
                                                     ABSOLUTE);
    solid->AddFacet(facet);
  }
}
