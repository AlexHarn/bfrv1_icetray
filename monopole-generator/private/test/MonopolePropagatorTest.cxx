...
#include <monopole-propagator/I3MonopoleRelativisticUtils.h>
#include <monopole-propagator/I3MonopolePropagator.h>
 ...
 */
I3ParticlePtr GenTestPart(double zen, double azi, double beta, double time, double length){
...
TEST_GROUP(monopole-propagator);
...
TEST(TestPropUtilsBasic)
{
  ...
  I3ParticlePtr testEnd = I3MonopoleRelativisticUtils::PropagateFastMonopole(testStart,mp_mass,true,true,true,NAN,10,0.001);
  ...
  I3ParticlePtr testEnd2 = I3MonopoleRelativisticUtils::PropagateFastMonopole(testStart,mp_mass,false,false,true,100,10,0.001);
...
{
  double testLen = I3MonopoleRelativisticUtils::CalculateNextLength(1e7,1e6,10,0.001);
  ENSURE_DISTANCE(testLen,7.5,0.000001);
  testLen = I3MonopoleRelativisticUtils::CalculateNextLength(2.3e8,1e7,100,0.001);
  ENSURE_DISTANCE(testLen,100.0,0.000001);
  testLen = I3MonopoleRelativisticUtils::CalculateNextLength(1.0001e6,1e6,10,0.0001);
  ENSURE_DISTANCE(testLen,0.0001,0.000001);
  //Finally, check one that should log_fatal
  bool passed = true;
  try{
    testLen = I3MonopoleRelativisticUtils::CalculateNextLength(NAN,NAN,NAN,NAN);
  }
  catch(...){
    passed=false;
  }
...
namespace mp_prop_io_test
{
  ...
  public :
    NotJustMPTreeTest(const I3Context& ctx) : I3Module(ctx) { AddOutBox("OutBox"); }
    void Physics(I3FramePtr frame) {
	...
	GetPrimaries(testTree);
      ...
      for(tree_iter=testTree.begin();
	  tree_iter!=testTree.end();
	  tree_iter++){
	if(HasParent(testTree,*tree_iter)){
      ...
      ENSURE_EQUAL(num_mp_child3,1+ceil((2*DIST_FROM_CENTER_MAX_TEST-10-MIN_LENGTH_TEST)/(MAX_LENGTH_TEST)));
...
TEST(NotJustMonopoles)
{
  ...
  tray.AddService("I3EmptyStreamsFactory","source");
  tray.AddService("I3GSLRandomServiceFactory","random");

  tray.AddModule("I3Muxer","muxer");
  tray.AddModule("mp_prop_io_test::CreateNotJustMPTree","test-source");
...
{
  class CreateMPTree : public I3Module{
  public :
    CreateMPTree(const I3Context& ctx) : I3Module(ctx) { AddOutBox("OutBox"); }
    void Physics(I3FramePtr frame){
      I3DoublePtr mp_mass_ptr(new I3Double(1e11*I3Units::GeV));
      frame->Put("MP_Mass",mp_mass_ptr);
      I3MCTreePtr testTree(new I3MCTree);

      I3ParticlePtr testmp1 = GenTestPart(0,0,.99,0,10*STEPSIZE_TEST);
      testmp1->SetType(I3Particle::Monopole);
      testmp1->SetPos(0,0,DIST_FROM_CENTER_MAX_TEST);
      testmp1->SetEnergy((mp_mass_ptr->value)/sqrt(1-.99*.99));
      AddPrimary(testTree,*testmp1);
  ...
  public :
    MPTreeTest(const I3Context& ctx) : I3Module(ctx) { AddOutBox("OutBox"); }
    void Physics(I3FramePtr frame) {
	...
	GetPrimaries(testTree);
      ...
      ENSURE_EQUAL(primaries[0].GetZ(),DIST_FROM_CENTER_MAX_TEST);
...
I3_MODULE(mp_prop_step_test::CreateMPTree);
I3_MODULE(mp_prop_step_test::MPTreeTest);
/**
 * This test is designed to simply check that step size can be set and over-ride the max/min length
 */
...
TEST(StepSizeTest)
{
  ...
  tray.AddModule("I3Muxer","muxer");
  tray.AddModule("mp_prop_step_test::CreateMPTree","test-source");
  tray.AddModule("I3MonopolePropagator","prop")
    ("StepSize",STEPSIZE_TEST)
    ("MaxLength",30*STEPSIZE_TEST) //Just make sure that it ignores this
    ("MaxDistanceFromCenter",DIST_FROM_CENTER_MAX_TEST);
  tray.AddModule("mp_prop_step_test::MPTreeTest","test");
...
//Helper function to check logfatal tests
void TestingLogFatal(double maxlen, double minlen, bool calcEn, bool calcDn, bool shouldPass){
  ...
  tray.AddModule("I3MonopoleGenerator","mono-gen")
    ...
    ("Disk_rad",0)
    ("Rand_dir",true);
  tray.AddModule("I3MonopolePropagator","mono-prop")
    ("CalculateEnergy",calcEn)
    ("CalculateDensityCorrection",calcDn)
    ("MaxLength",maxlen)
    ("MinLength",minlen);
  tray.AddModule("TrashCan","trash");
...
TEST(LogFatalTest)
{
  TestingLogFatal(10,1,false,true,false);
  TestingLogFatal(1,10,true,true,false);
  //Finally, do one run just to make sure everything works when it should
  TestingLogFatal(10,1,true,true,true);
}
