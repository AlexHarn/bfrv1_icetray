...
void I3PhotoSplineTable::GetKnotVector(double *knots, int dimension)
{
	int n_knots=tablestruct_->nknots[dimension];

	for (int i=0; i < n_knots;++i)
	{
		knots[i]=tablestruct_->knots[dimension][i];
	}

}

void I3PhotoSplineTable::GetNumKnots(long &num_knots, int dimension)
{
	num_knots=(tablestruct_->nknots[dimension]);

}

void I3PhotoSplineTable::GetAxisOrder(int &order, int dimension)
{
	order=(tablestruct_->order[dimension])+1;
...
/* get the effective coefficients along a given dimension , effectively slicing the n-d spline*/
int
I3PhotoSplineTable::GetSliceCoefficients(double *coordinates, double *results, int dimension, int derivative, int area_norm)
{
	// should be done only once for a table
	int centers[tablestruct_->ndim];
	...
	if (tablesearchcenters(&*tablestruct_, coordinates, centers) == 0) {
		if(derivative>0)
		{
			ndsplineeval_slice_coeffs(&*tablestruct_, coordinates, centers, results, dimension, 1 << dimension, area_norm);
		}
		else
		{
			ndsplineeval_slice_coeffs(&*tablestruct_, coordinates, centers, results, dimension, 0, area_norm);
		}
...
I3PhotoSplineTable::EvalHessian(double *coordinates, double result[6][6])
{
	...
	int centers[tablestruct_->ndim];
	unsigned order[tablestruct_->ndim];

	if (tablesearchcenters(&*tablestruct_, coordinates, centers) != 0)
		return EINVAL; 

	std::fill(order, order+tablestruct_->ndim, 0);
	for (i = 0; i < ndim; i++) {
		// For diagonal entries, we need the second derviative
		order[i] = 2;
		result[i][i] = ndsplineeval_deriv(&*tablestruct_, coordinates,
		    centers, order);
		...
		for (j = 0; j < i; j++) {
			...
			result[i][j] = result[j][i] =
			    ndsplineeval_deriv(&*tablestruct_, coordinates,
			    centers, order);
			order[j] = 0;
		}
		order[i] = 0;
