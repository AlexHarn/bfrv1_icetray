    ...
    rawYield_(0.)
{
	...
	AddParameter("TiltTableDir", "Path to the ice layer tilt table files.", "");
...
I3PhotoSplineService::I3PhotoSplineService(const std::string& amplitudeTable,
    const std::string& timingTable, double timingSigma, const std::string& tiltTableDir, double maxRadius) :
    I3PhotonicsService(), I3ServiceBase("I3PhotoSplineService"),
    ...
    std::string timeFileName)
{
	...
	if(timingSplineTable_->GetGeometry() != 
	   amplitudeSplineTable_->GetGeometry())
		log_fatal_stream("Timing table " << timeFileName 
		                 << " has different geometry than amplitude table "
		                 << ampFileName);

	geometry_ = amplitudeSplineTable_->GetGeometry();
	
	if(timingSplineTable_->GetGeometryType() != 
	   amplitudeSplineTable_->GetGeometryType())
		log_fatal_stream("Timing table " << timeFileName 
		                 << " has different geometry type than amplitude table "
		                 << ampFileName);

	geotype_ = amplitudeSplineTable_->GetGeometryType();

	if(timingSplineTable_->GetNGroupTable() != 
	   amplitudeSplineTable_->GetNGroupTable())
		log_fatal_stream("Timing table " << timeFileName 
		                 << " has different n_group than amplitude table "
		                 << ampFileName);

	nGroup_ = timingSplineTable_->GetNGroupTable();

	if(timingSplineTable_->GetParity() != 
	   amplitudeSplineTable_->GetParity())
		log_fatal_stream("Timing table " << timeFileName 
		                 << " has different parity than amplitude table "
		                 << ampFileName);
    ...
    bool timing)
{
	...
	double dz = tilt.zshift(xOM_/I3Units::m, yOM_/I3Units::m, zOM_/I3Units::m);
	...
	case POINTSOURCE:
		...
		tablecoordinates[4+timegap] = coordPxC_[4] - dz;
		break;
	...
	case INFINITEMUON:
		...
		tablecoordinates[3+timegap] = coordPxC_[4] - dz -
...
void
I3PhotoSplineService::GetKnotVector(double *knots, int dimension)
{
	timingSplineTable_->GetKnotVector(knots, dimension);
}

void
I3PhotoSplineService::GetNumKnots(long &num_knots, int dimension)
{
	timingSplineTable_->GetNumKnots(num_knots,dimension);
}

void
I3PhotoSplineService::GetAxisOrder(int &order, int dimension)
{
	timingSplineTable_->GetAxisOrder(order,dimension);
}
...
void
I3PhotoSplineService::GetTimeSliceCoefficients(double *results, int time_dimension, int derivative, int area_norm)
{
	// get the coefficients in time for a given slice in the other coordinates
	
	double tablecoordinates[6];

	FillTableCoordinates(tablecoordinates, true);

	// Passing the time dimension on seems to be the working option atm, geotype did not work for infinite muon splines
	/*
	switch (geotype_) {
	case POINTSOURCE:
		time_axis = 3;
	...
	// some arbitrary number for the time .. doesnt matter
	tablecoordinates[time_dimension] = 1000;

	timingSplineTable_->GetSliceCoefficients(tablecoordinates, results, time_dimension, derivative, area_norm);
}
