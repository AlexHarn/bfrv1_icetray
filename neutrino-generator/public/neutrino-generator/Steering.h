...
class Steering : public I3ServiceBase {
 ...
 public:
   ...
   *  DETECTOR - final interaction only
   */
  ...
  inline const int    GetNSteps()     const { return nsteps_; }
   ...
   */
  inline const std::vector<double> & GetGlobalXsecScalesVec() const { return global_xsec_scales_vec_; }
   ...
   */
   ...
   void SetGlobalXsecScalesVec(std::vector<double> v) { global_xsec_scales_vec_ = v; }
   ...
   void SetStepSize(double t) { stepsize_ = t; }
 ...
 private :
  ...
  /**
   * these parameters are used to calculate column 
   * depth etc.
   */

  /**
   * initial number of steps for propagation in Earth
   */
  int    nsteps_;       
