...
TEST(weighted_interaction_test){
  ...
  for (int i=0; i<nevts; ++i) {
     ...
     InteractionType itype = interaction->MakeInteraction(p, false, ccratio, ncratio, grratio);
     //std::cerr << "particle type " << p->GetType() << " interaction type " << itype << " : particle energy is " << p->GetEnergy() << std::endl;
  ...
  ENSURE(numgr != 0); // you must have some gr interaction for 3PeV
...
TEST(calc_total_columndepth_test){
  ...
  using namespace nugen;
  boost::shared_ptr<I3RandomService>  random = nugen::test::make_random();
  boost::shared_ptr<Steering> steer = nugen::test::make_steering("IceSheet", DETECTOR);
  boost::shared_ptr<I3NuGInteractionInfo> interaction = nugen::test::make_interaction(random, steer);
