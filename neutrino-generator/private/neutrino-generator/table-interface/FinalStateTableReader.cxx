...
void FinalStateTableReader::ReadFinalStateTableFillArray(const string& infile){
   ...
   if(Imax>MaxEnergyBins)log_fatal("table index too large for number of steps in the energy range Imax = %d, MaxEneBins = %d",Imax,MaxEnergyBins);
   if(Nfinal>MaxFinalStates)log_fatal("table index too large for number of final state, Nfinal = %d, MaxFinalState = %d",Nfinal,MaxFinalStates);
   if(Ndim>MaxDimensions)log_fatal("table index too large for number of dimension of final state, Ndim = %d, MaxDim = %d", Ndim,MaxDimensions);
