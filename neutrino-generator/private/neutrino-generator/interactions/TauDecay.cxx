...
void TauDecay::FillDaughterParticles(ParticlePtr tau_ptr, double decay_energy){
  //*************************************************************
  // Comments about the generated secondary particles
  // K.Hoshina (Mar 16 2018)
  // This function may generate secondaries such as
  // Tau -(decay)-> NuTau + EMinus + Hadron
  // which brakes conservation of lepton number.
  // The authors have been aware of the issue but agreed that it is OK 
  // because these charged secondary particles are never used for our analysis.
  // We swtch on the TauDecay process only inside the Earth during propagation, and
  // switch off the process at detector, because TauDecay is managed in subsequent modules.
  // However, for some reasons, if you turn on TauDecay *AT* detector, 
  // this function must be changed.
  //
  // In practical, the chanel like
  // Tau -(decay)-> NuTau + EMinus + Hadron
  // represents 
  // Tau -(decay)-> NuTau + Hadron + Hadron
  // Current TauDecay table doesn't have two entries for hadrons,
  // and you may need to update the table structure if you want to 
  // fix the issue.
  //*************************************************************
  ...
  //Possible final states: nu_tau, nu_mu, nu_e, hadron, electron, muon
