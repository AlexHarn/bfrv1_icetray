...
void InteractionGR::FillDaughterParticles(ParticlePtr nuin_ptr, 
                                          double)
{
  // Because Grashow resonance happens only around PeV energy,
  // here we ignore lepton mass (set total energy instead of kinetic energy).
  // Also, we assume outgoing lepton (and hadron) angles are same as
  // parent neutrino, because at this energy secondaries are ejected to
  // forward in LAB frame.
  ...
  // Mar.16 2018 by K.Hoshina
  // modified dimension of final state from 7 to 8, now W->Hadrons process
  // generates two secondary hadrons instead of big one hadron, 
  // as LeptonInjector does.
  //double finalstate[7]={0.,0.,0.,0.,0.,0.,0.}; // init. final state kinematic
  double finalstate[8]={0.,0.,0.,0.,0.,0.,0.,0}; // init. final state kinematic
  FillResonanceStates(finalstate);
  int n_daughters=0;
  // make the final state particles
  // 0) anu_e; 1) e; 2) anu_mu; 3) mu; 4) anu_tau; 5) tau; 6) hadrons;
  /*
...
*/
  if(finalstate[0] > 0) {
   ...
   //create neutrino
    ...
    //create second hadron
    ParticlePtr hadron_daughter_particle2(new Particle(I3Particle::Null, I3Particle::unknown, steer_));
    ++n_daughters;
    hadron_daughter_particle2->SetShape(I3Particle::Cascade);
    hadron_daughter_particle2->SetFitStatus(I3Particle::NotSet);
    hadron_daughter_particle2->SetPos(nuin_ptr->GetEndPosition());
    hadron_daughter_particle2->SetTime(nuin_ptr->GetTime()+nuin_ptr->GetLength()/nuin_ptr->GetSpeed());//7
    hadron_daughter_particle2->SetDir(nuin_ptr->GetDir());//8
    hadron_daughter_particle2->SetLength(NAN);//9
    hadron_daughter_particle2->SetSpeed(I3Constants::c);//10
    hadron_daughter_particle2->SetType(I3Particle::Hadrons);
    hadron_daughter_particle2->SetEnergy(finalstate[7]*nuin_ptr->GetEnergy());
    hadron_daughter_particle2->CopyExtraInfoFrom(*nuin_ptr);

    nuin_ptr->AddDaughter(hadron_daughter_particle2);
...
// 0) anu_e; 1) e; 2) anu_mu; 3) mu; 4) anu_tau; 5) tau; 6) hadrons;
// //this is wrong : 0) anu_e; 1) nu_e; 2) anu_mu; 3) nu_mu; 4) nu_tau; 5) tau; 6) hadrons;
////////////////////////////////////////////////////////////////////////
...
void InteractionGR::DebugPrint(string fname, int n) {
  ...
  double elog = elogmax;
  unsigned int ntrials = n;

  FILE *out;
  out = fopen(fname.c_str(), "w");
  
  fprintf(out, "#prim_elog, total_xsec, secondary_type, y\n");

  while (elog >= elogmin) {
     double nu_elog = elog;
     double nu_e = pow(10., nu_elog); 
     double total_xsec = GetXsecCGS(nu_e);

     unsigned int i, j;
     for (i=0; i<ntrials; ++i) {
        double finalstate[8]={0.,0.,0.,0.,0.,0.,0.,0}; 
        FillResonanceStates(finalstate);
        
        for (j=0; j<8; ++j) {
           if(finalstate[j] > 0) {
              fprintf(out, "%e %e %d %e\n", nu_elog, total_xsec, int(j), finalstate[j]);
           }
        }
     }
     elog -= delog;
