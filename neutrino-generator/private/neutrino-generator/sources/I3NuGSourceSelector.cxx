...
void I3NuGSourceSelector::DAQ(I3FramePtr frame)
{
  ...
  //std::cout << "I3NuGSourceSelector : number of neutrino candidates is " << nuvec.size() << std::endl;
  ...
  if (nuvec.size() > 0) {
     ...
     std::pair<bool, std::pair<ParticlePtr, double> > apair = 
             PickupANeutrino(random_, nuvec, frame, useZeVForm_);

     if (apair.first == true) {
         // pickup succeed and only one InIce neutrino is selected.
         nup = apair.second.first;
         double selection_weight = apair.second.second;
         InteractionInfo& intinfo = nup->GetInteractionInfoRef();
         intinfo.SetPrimarySelectionWeight(selection_weight);

         log_debug("Selected Neutrino : Type %s, log10E %e, Selection Weight %e",
...
{
   // If the neutrinos that don't intersect the sampling surface
   // return 0 so that this neutrino will not be selected.
   I3Surfaces::SurfaceConstPtr detectionVolume = steer_ptr_->GetDetectionSurface();
   if (detectionVolume &&
           !std::isfinite(detectionVolume->GetIntersection(
           p->GetPos(), p->GetDir()).first)) {
      return 0;
   } 
