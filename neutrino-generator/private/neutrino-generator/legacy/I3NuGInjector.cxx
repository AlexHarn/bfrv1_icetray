                                          ...
                                          I3MapStringDoublePtr wmap)
{
  ...
  if (simulate_pointsource_) {
     SourceUtils::FillPointPrimaryGenInfo(neutrino_ptr,
                                  steer_,
                                  gamma_,
                                  energy_min_log_,
                                  energy_max_log_,
     ...
     SourceUtils::FillDiffusePrimaryGenInfo(neutrino_ptr,
                                  steer_,
                                  gamma_,
                                  energy_min_log_,
                                  energy_max_log_,
...
bool I3NuGInjector:: DecideEnergy(ParticlePtr neu_ptr){
  ...
  // because I confirmed this function gives exactly 
  // same result with SourceUtils::SetPowerLowEnergy,
  // I replace the function to the one.
  return SourceUtils::SetPowerLowEnergy(neu_ptr, gamma_,
                        energy_min_log_, energy_max_log_,
                        random_);
      ...
      if (zenith_min_ == zenith_max_) {
        ...
        if (zenith_sampling_ == COS) {
           ...
           std::vector<double> result = 
              ZenithSampler::SimpleSlopeSampler(
                    zenith_weight_param_,
                    zenith_min_, zenith_max_, random); 
           Zenith = result[0]; 
           weight = result[1]; 
