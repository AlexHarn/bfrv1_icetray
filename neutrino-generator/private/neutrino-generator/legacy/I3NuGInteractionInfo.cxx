...
I3NuGInteractionInfo::I3NuGInteractionInfo(const I3Context &c):
  ...
  tablesDir_(""),
  xsec_model_("csms"),
  energyMin_(Defaults::energyMin),
  energyMax_(Defaults::energyMax),
...
//_____________________________________________________________
void I3NuGInteractionInfo::PrintSetting()
{
   log_info("NuGen InteractionInfo Setting");
   log_info("-------------------------------------------");
   log_info("TablesDir            %s", tablesDir_.c_str());
   log_info("CrossSectionModel    %s", xsec_model_.c_str());
   log_info("MinEnergy            %f", energyMin_);
   log_info("MaxEnergy            %f", energyMax_);
   log_info("===========================================");
}
...
void I3NuGInteractionInfo::ReadInteractionFiles()
{
  ...
  for(; iter != interactionVect_.end(); ++iter){
     ...
     if (energyMin_ < int_ptr->GetMinEnergy()) {
        energyMin_ = int_ptr->GetMinEnergy();
     }
     if (energyMax_ > int_ptr->GetMaxEnergy()) {
        energyMax_ = int_ptr->GetMaxEnergy();
     }

     log_trace("Cross-section file: %s", int_ptr->GetCrosssectionFileName().c_str());
  }
...
//
  ...
  if (energy < energyMin_ || energy > energyMax_) {
     log_info("Return NOINTERACTION because nu Energy %g is out of range:(%g, %g)",
               energy, energyMin_, energyMax_);
  ...
  */
  ...
  if (isfinal) {
     // this is final interaction.
     // set interaction type weight.
     nuin_ptr->GetInteractionInfoRef().SetInteractionTypeWeight(intweight);
 ...
 */
  ...
  if (prev_state_columndep_.UsePrev(state)) {
     ...
     log_trace("previous params :"
            "entpos(%g,%g,%g), extpos(%g,%g,%g), len=%g, particle=%p",
