...
bool I3NuGEvent::PropagateNeutrino(ParticlePtr particle_ptr){
  ...
  for(int istep=0; istep < NSTEPS; ++istep) {
     ...
     if(thisStepInteractionProbability > temprandom) {
        ...
        InteractionType int_type = interaction_ptr_->MakeInteraction(particle_ptr, false);
...
void I3NuGEvent::MakeFinalInteraction(ParticlePtr neu_part)
{
  ...
  InteractionType int_type = interaction_ptr_->MakeInteraction(neu_part, true);
...
void I3NuGEvent::SetFinalInteractionPosition(ParticlePtr particle_ptr)
{
  ...
  const double column_depth  = 
    interaction_ptr_->CalculateTotalColumnDepth(startPosCE, interaction_positionCE);
  ...
  log_trace("   ||total crosssection %.2f x10^-12 [mb]", total_crosssection*std::pow(10.,12.));
  log_trace("   ||Total interaction probabiliby weight = %.5f x10^-6", probability*std::pow(10., 6.));		
...
double I3NuGEvent::CalculateMaximumTotalInteractionProbability(ParticlePtr particle_ptr)
{
  ...
  log_trace("   ||total crosssection %.2f x10^-12 [mb]", total_crosssection*std::pow(10.,12.));
  log_trace("   ||Max total-interaction probabiliby = %.5f x10^-6", probability*std::pow(10., 6.));		
