...
I3NeutrinoGenerator::I3NeutrinoGenerator(const I3Context& ctx) : 
  I3ConditionalModule(ctx), I3NeutrinoGeneratorBase(Defaults::propagationMode)
{
  ...
  AddParameter("PrimaryNuName", 
               "If a frame contains an I3Particle with this name, NuGen takes "              
               "the particle as primary and propagate it. By default, the PrimaryNuName "
               "is set to 'NuGPrimary', which is the default output-name of I3NuGDiffuseSource."
               "If the particle does not exist in frame, NuGen uses I3NuGInjector to generate a "
               "primary particle.",
                primary_nu_name_ );       
  ...
  // default is defined in its mother class.
  // prop_mode_ = Defaults::propagationMode;
  ...
  intpos_sample_opt_ = 1;  // must be 1 for correct simulation
...
{ 
  ...
  GetParameter("InteractionPositionOpt", intpos_sample_opt_);
  GetParameter("InteractionWeightOpt",   interact_weight_opt_);
  GetParameter("CrosssectionColumndepthOpt", crosssectionxcolumndepth_opt_);
  ...
  if (!interaction_) {
    interaction_ = context_.Get<I3NuGInteractionInfoDifferentialPtr>(interaction_name_);
  }
  if (!interaction_) {
  ...
  I3NeutrinoGeneratorBase::PrintSetting();
...
void I3NeutrinoGenerator::DAQ(I3FramePtr frame)
{
  ...
  if (frame->Has(primary_nu_name_)) {
     ...
     if (context_.Has(injector_name_)) {
        injector_ = GetService<I3NuGInjectorPtr>(injector_name_);
     } else {
        log_error("No primary neutrino named %s and no injector service exist."
                  " Need to add a primary particle or set injector service.", 
