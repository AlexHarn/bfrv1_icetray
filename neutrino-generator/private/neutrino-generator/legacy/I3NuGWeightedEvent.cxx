...
I3NuGWeightedEvent::I3NuGWeightedEvent(
		     ...
		     I3RandomServicePtr random,
                     ...
                     int crosssection_cdep_opt,
                     int impactparam_opt) :  
  I3NuGVEvent(propmode, intpos_sample_opt, interact_weight_opt, crosssection_cdep_opt, impactparam_opt), 
  Pickup(),
  random_(random), 
                                                 ...
                                                 bool skipFinalInteraction)
{
  ...
  if (! (steer_ptr_->GetSimMode() == DETECTOR)) {
     initial_interacted = PropagateNeutrino(initial_neu_ptr);
  } else {
     // this is DETECTOR mode, so initial_neu_ptr must be inice neutrino
     SetInIceParticle(initial_neu_ptr);
  }
  ...
  if( (!initial_interacted) ){
     log_debug("initial_primary is not interacted, fill final interaction with probability");

     if (skipFinalInteraction) {
        // the primary neutrino is inice neutrino
        SetInIceParticle(initial_neu_ptr);
  ...
  log_debug("      initial_primary interacted, secondary created");
  ...
  log_debug("Number of in-Earth %d", n_in_earth);
  ...
  for (int i=0; i< n_in_earth; ++i) {
     ...
     if (!currentPart->InteractedOrDecayed()) {       
        ...
        if(currentPart->IsNeutrino() && 
           ...
           log_trace("this is candidate for InEarthNu");
           finalInteractionNuCandidates_.push_back(currentPart);
        } else {
           log_trace("This particle is not neutrino, not candidate");
        }
     } 
     I3Position p = earthservice_ptr_->GetEarthCoordPosFromDetCoordPos(currentPart->GetEndPosition()); 
     log_trace("This particle %s is already interacted at r=%f",
  ...
  if (finalInteractionNuCandidates_.size()) {
     ...
     log_debug("got %zu possible primaries.", finalInteractionNuCandidates_.size());
     ...
     // this is new code
     //
     pair<bool, pair<ParticlePtr, double> > picked = PickupANeutrino(random_,
                            finalInteractionNuCandidates_, frame);
     if (! picked.first) {
        log_debug("All InIce candidates don't match the condision writtern in Selection Weighter. return false now");
        return false;
     }

     ParticlePtr final_inice = picked.second.first;
     double selection_weight = picked.second.second;
     ...
     log_debug("FinalInIce particle : propagation weight %e, selection weight %e", prop_weight, selection_weight);
     final_inice->SetPropagationWeight(prop_weight*selection_weight);
     SetInIceParticle(final_inice);

     // if skipFinalInteraction is True, set "InIce" location to all arrival neutrinos
     // and return before processing final interaction.
     if (skipFinalInteraction) {
...
//
  ...
  int      nsteps = steer_ptr_->GetNSteps();
  double   dx = propLength/nsteps;
  double   mindx = steer_ptr_->GetStepSize();
  if (dx < mindx) {
  ...
  for(int istep=0; istep < nsteps; ++istep) {
     ...
     if (temprandom < thisstepinteractionprobability) {
        ...
        if (propagationMode_ == nugen::NOPROPWEIGHT || 
            (propagationMode_ == nugen::AUTODETECT && particle_ptr->IsTauFlavor()))  {
...
void I3NuGWeightedEvent::MakeFinalInteraction(ParticlePtr neu_part)
{
  ...
  if(! neu_part->IsGeoCalculated()) {
      log_error("Geom calculation failed, return false now"); 
      return;
  }

  // first, check energy range.
  double nu_ene = neu_part->GetEnergy();
  if (nu_ene < interaction_ptr_->GetMinEnergy() || 
      nu_ene > interaction_ptr_->GetMaxEnergy() ) {

     log_warn("InIce neutrino energy %g is out of range(%g,%g). Return now.", nu_ene, interaction_ptr_->GetMinEnergy(), interaction_ptr_->GetMaxEnergy());

     // reset InIce tag to Anywhere for the selected neutrino.
     // PROPOSAL and cmc skips the event if no InIce
     // particle exists in the MCTree.
     neu_part->SetLocationType(I3Particle::Anywhere);
     // also, set column depth and interaction weight 
     // as 0
     InteractionInfo &intinforef = neu_part->GetInteractionInfoRef();
     intinforef.SetTotalColumnDepthCGS(0);
     intinforef.SetInteractionWeight(0);
     intinforef.SetInteractionXsecCGS(-1);
     return;
  }
  ...
  InteractionType int_type = 
            interaction_ptr_->MakeInteraction(neu_part, true, 
                                              interaction_cc_factor_, 
                                              interaction_nc_factor_, 
                                              interaction_gr_factor_);

  log_debug("Final interaction type is %d",int_type);
  
  log_debug("%s", (neu_part->GetInteractionInfoRef().PrintInteractionInfo()).c_str());
...
//
  ...
  if (crosssectionxcolumndepth_opt_ == 0) {
     ...
     if (crosssectionxcolumndepth_opt_ == 0) {
        ...
        // (g/m3)*m = g/m2
        total_column_depth = 
           interaction_ptr_->CalculateTotalColumnDepth(startPosCE, endPosCE);
     }
  ...
  } else if (crosssectionxcolumndepth_opt_ == 1) {
  ...
  if (interact_weight_opt_ == 0) {
  ...
  } else if (interact_weight_opt_ == 1) {
     ...
     // If exponential factor is greater than EXPTERM_THRESHOLD, 
     // simple approximation won't work.
     // use UHE compatible form.
     if (exp_factor_tot > EXPTERM_THRESHOLD) {
...
double I3NuGWeightedEvent::CalculateMaximumTotalInteractionProbability(ParticlePtr particle_ptr)
{
  ...
  if (exponential_factor > EXPTERM_THRESHOLD) {
     // here is UHE form
