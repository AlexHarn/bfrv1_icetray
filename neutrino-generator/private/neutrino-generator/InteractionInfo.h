...
class InteractionInfo {
 ...
 public:
  ...
  InteractionInfo() : vertex_(0,0,0), vertexTime_(-1),
                      ...
                      intposweight_(1.0), intweight_(1.0), inttypeweight_(1.0), ninicenus_(0){}
  ...
  void SetWeights(const InteractionInfo &intInfo) 
  {
     ...
     inttypeweight_ = intInfo.inttypeweight_; 
  }
  ...
  void FillWeights(I3MapStringDoublePtr wmap) const
  {
     ...
     (*wmap)["InteractionTypeWeight"]  = inttypeweight_;
  ...
  void RestoreWeights(I3MapStringDoubleConstPtr wmap) 
  {
     ...
     inttypeweight_ = GetWeightValue(wmap, "InteractionTypeWeight");
  }
  ...
  void RestoreInteractionInfo(I3MapStringDoubleConstPtr wmap) 
  {
     ...
     Utils::GetMapValue(*wmap, std::string("NInIceNus"), ninicenus_);
  }
  ...
  {
     return GetTotalPrimaryWeight() * propweight_ * intposweight_ * intweight_ * inttypeweight_;
  }
  ...
  inline const double GetInteractionTypeWeight()   const {return inttypeweight_;}
  inline const double GetNInIceNus()           const {return ninicenus_;}
  ...
  inline void SetInteractionTypeWeight(double w) {inttypeweight_ = w;}
  inline void SetNInIceNus(double w) {ninicenus_= w;}

  std::string PrintInteractionInfo() {
     std::ostringstream os;
     os << "TotalWeight:" << GetTotalWeight()
      << " InteractionWeight:" << intweight_
      << " InteractionTypeWeight:" << inttypeweight_
      << " SelectionWeight:" << selectionweight_
      << " PropagationWeight:" << propweight_;
     return os.str();
  }
 ...
 private:
  ...
  /**
   * @brief weight for interaction type, when you limit
   * interaction type (e.g. allows CC interaction only)
   */
  double inttypeweight_;

  /**
   * @brief number of neutrinos reached to detector.
   * Only one of them interacts.
   */
  double ninicenus_;
