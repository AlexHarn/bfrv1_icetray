#ifndef IPDF_PANDEL_GSemiConvolutePEP_H
#define IPDF_PANDEL_GSemiConvolutePEP_H
...
/**
    ...
    The integral of the PDF is computed without convolution, using the
    analytical expression of the integral of plain Pandel.
...
class GSemiConvolutePEP : public PEPBase< GSemiConvolutePEP<IceModel> > {
public:
  typedef GSemiConvolutePEP<IceModel> Self;
  ...
  /// (root-cint doesn't recognise a new in a default parameter)
  explicit GSemiConvolutePEP(IceModelPtr ice=IceModelPtr(new IceModel()),
  ...
  inline double intPandelFunction(const double delay,
                      const double propd,
...
template<typename IceModel>
const double GSemiConvolutePEP<IceModel>::DEFAULT_SIGMA=15.;

template<typename IceModel>
inline GSemiConvolutePEP<IceModel>::GSemiConvolutePEP(
...
template<typename IceModel>
template<typename OmReceiver>
inline void GSemiConvolutePEP<IceModel>::setOmHardware(const OmReceiver& omr) const {
...
template<typename IceModel>
template<typename PEHit, typename EmissionHypothesis>
inline double GSemiConvolutePEP<IceModel>::getLogPdf(
    const PEHit& pehit,
...
template<typename IceModel>
template<typename OmReceiver, typename EmissionHypothesis>
inline double GSemiConvolutePEP<IceModel>::
...
template<typename IceModel>
inline std::ostream& operator<<(std::ostream& os, const IPDF::Pandel::GSemiConvolutePEP<IceModel>&) {
  return (os<<"GSemiConvolutePEP<"<<IceModel::name()<<">");
}

#endif // IPDF_PANDEL_GSemiConvolutePEP_H
