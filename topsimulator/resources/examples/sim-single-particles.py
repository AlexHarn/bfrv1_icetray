...
parser.add_option("-p", "--particle", action="store", type="string", dest="particle",
                  help="Particle ID. Any I3Particle.ParticleType string (MuPlus, MuMinus, EPlus, EMinus, Gamma, KPlus, etc). The following are also accepted for backward compatibility: mu+, mu-, e+, e-, gamma, p+, p-, n. Default: MuMinus-", metavar="id", default='MuMinus')
...
parser.add_option("--run", action="store", type="int", dest="run_id", help="Run number",
                  metavar="N", default=0)
parser.add_option("--event", action="store", type="int", dest="event_id", help="Event number",
                  metavar="N", default=0)
