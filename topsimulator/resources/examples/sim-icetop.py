...
parser.add_option("--pecompress", action="store", type="int", dest="pecompress",
                  ...
                  default=2)
parser.add_option("--correct-obs-level", action="store_true", dest="correct_obs_level",
                  help="Automatically raise the observation level of a CORSIKA shower if it happens to lie below the snow at the location of any tank", default=False)
parser.add_option("--raise-obs-level", action="store", type="float", dest="raise_obs_level",
                  help="Raise the observation level of particles in a CORSIKA shower by a fixed amount (in meters)",
                  default=0.)
