        ...
        with rep.section(key):
            ...
            try:
                if keyBasedCmp and key in keyBasedCmp:
                    keyBasedCmp[key](rep, cur, ref, **kwargs)
                    continue
                Cmp(rep, cur, ref, **kwargs)
                continue
            except:
                rep.exception()


def CmpMap(rep, cur, ref, **kwargs):
    ...
    for key in ref.keys():
        if key in cur:
            Cmp(rep, cur[key], ref[key], **kwargs)
        else:
            rep.failure(str(key) + " missing")
    else:  # if map is empty...
        rep.success()

def CmpSeries(rep, cur, ref, **kwargs):
...
def CmpEqual(rep, cur, ref, **kwargs):
    """Compare equality. NaN == NaN yields true."""
    from math import isnan
    isEqual = cur == ref
    if isinstance(ref, float):
        isEqual |= isnan(ref) and isnan(cur)
        if 'tolerance' in kwargs.keys(): isEqual |= (abs(ref-cur) < kwargs['tolerance']*abs(ref+cur))
    if isEqual:
        rep.success()
    else:
        msg = ""
        ...
        if isinstance(cur, dataclasses.I3Direction):
            print(cur.zenith, ref.zenith)
            print(cur.azimuth, ref.azimuth)
        rep.failure(msg)


def CmpFields(rep, cur, ref, *fields, **kwargs):
    """Compare two objects based on their field values, using equality."""
    for field in fields:
        CmpEqual(rep, getattr(cur, field), getattr(ref, field), **kwargs)


def CmpI3Position(rep, cur, ref, **kwargs):
    """Compare I3Position (or I3Direction) comparing only (x,y,z)"""
    CmpFields(rep, cur, ref,"x","y","z", **kwargs)


def CmpI3Particle(rep, cur, ref, **kwargs):
    """Compare fields of I3Particle, excluding random ParticleID."""
    CmpFields(rep, cur, ref,
              "energy", "fit_status", "length",
              "location_type", "pdg_encoding", "shape",
              "speed", "time", "type", **kwargs)
    CmpI3Position(rep, cur.pos, ref.pos, **kwargs)
    CmpI3Position(rep, cur.dir, ref.dir, **kwargs)

def CmpI3EventHeader(rep, cur, ref, **kwargs):
    """Compare fields of I3EventHeader, excluding random SubEventID."""
    CmpFields(rep, cur, ref,
              "data_stream", "end_time", "event_id", "run_id",
              "start_time", "sub_event_stream", "sub_run_id", **kwargs)


def CmpIteritems(rep, cur, ref, **kwargs):
    ...
    while True:
        ...
        Cmp(rep, curItem[1], refItem[1], **kwargs)

def CmpI3Tree(rep, cur, ref, **kwargs):
    """Compare trees by size and element-wise."""
    if len(ref) != len(cur):
        rep.failure("lengths differ (%i, %i)" % (len(ref), len(cur)))
        return
    ...
    while True:
        try:
            ...
            break
        Cmp(rep, curItem, refItem, **kwargs)
...
class GeneralComparison(object):
    ...
    def __call__(self, rep, cur, ref, **kwargs):
        ...
        if ref.__class__ in self._cmp:
            self._cmp[ref.__class__](rep, cur, ref, **kwargs)
            return
        ...
        if hasattr(ref, "__str__"):
            ...
            if ref.__str__ is not ref.__repr__:
                CmpEqual(rep, str(cur), str(ref), **kwargs)
                return
