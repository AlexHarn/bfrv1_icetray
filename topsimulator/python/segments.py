...
def SimulateNew(tray, name,
    ...
    tray.AddModule("PMTResponseSimulator","PMTResponseSimulator",
                   ...
                   Output = IceTopPESeriesName + '_Pulses',
                   RandomServiceName = RandomServiceName
                   #MergeHits = True
                   )
    tray.AddModule("DOMLauncher", "DOMLauncher",
                   Input = IceTopPESeriesName + '_Pulses',
#                   Output = "IceTopRawData",
                   SuperNovaMode = SuperNovaMode,
                   RandomServiceName = RandomServiceName
                   )
    tray.AddModule(DOMLauncher.launch_splitter, "split_launches", streams = [icetray.I3Frame.DAQ])
        ...
        if weighted:
            if injector == "normal":
                tray.AddService("I3InjectorFactory<I3CorsikaInjector>", "particle-injector",
                                ...
                                RaiseObservationLevel = raise_obs_level,
                                CorrectObservationLevel = correct_obs_level
                                )
            else:
