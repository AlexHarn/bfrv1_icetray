...
public:
  ...
  void Configure()
  {
    ...
    log_debug("Launches = %s, WaveformsIn = %s, PESeriesMap = %s, Components = %s, Waveforms = %s, Pulses = %s", 
	      launches_.c_str(), waveforms_in_.c_str(), pe_series_map_.c_str(), components_.c_str(), waveforms_.c_str(), pulses_.c_str());
  }
  ...
  void DAQ(I3FramePtr frame)
  {
    ...
    // KR debugs
    if (launches) log_debug("Launches size = %zu", launches->size()); else log_debug("NO LAUNCHES");
    if (waveforms_in) log_debug("Waveforms_in size = %zu", waveforms_in->size()); else log_debug("NO WAVEFORMS_IN");
    if (components) log_debug("Components size = %zu", components->size()); else log_debug("NO COMPONENTS");
    if (pe_series_map) log_debug("PEseries size = %zu", pe_series_map->size()); else log_debug("NO PESERIES");
    ...
    if (launches)
      ...
      binning = GetBinnings(waveforms_in); 

    for(unsigned int j = 0; j != binning.size(); ++j) {
      const OMKey& om = binning[j].om;
      log_debug("Bin %d: OM (%d, %d)", j, om.GetString(), om.GetOM());

      log_debug("Find it in the geometry...");
      ...
      // Skip it if it's not in IceTop
      if (geo->second.omtype == I3OMGeo::IceCube)
	continue;

      log_debug("Create the waveforms_out series");
      for (I3MapStringInt::const_iterator it = components->begin(); it != components->end(); ++it) {
	if (waveforms_out[it->second]->find(om) == waveforms_out[it->second]->end())
	  (*waveforms_out[it->second])[om] = I3WaveformSeries();
      }

      log_debug("Look for it in the I3MCPESeries");
      if (pe_series_map->find(om) == pe_series_map->end()) 
	log_fatal("This OM (%d %d) was in the input Lanches/Waveforms, but was not in the PESeriesMap!", om.GetString(), om.GetOM());
      ...
      log_debug("Start = %f, End = %f, Width = %f", start, end, width);
...
I3TopAddComponentWaveforms::GetBinnings(I3DOMLaunchSeriesMapConstPtr launches) const
{
  ...
  for (I3DOMLaunchSeriesMap::const_iterator launch_it = launches->begin(); launch_it != launches->end(); ++launch_it) {
    ...
    for (unsigned int i = 0; i != launch_it->second.size(); ++i) {
      ...
      const int trigtype = launch.GetTriggerType();
	...
	width = 128/ATWDSamplingRate(atwd_id, status->second, calib->second);
      ...
      if (trigtype == I3DOMLaunch::CPU_REQUESTED)
	log_info("OM (%d, %d): Skipping CPU_REQUESTED launch when adding components (presumably a beacon launch).", om.GetString(), om.GetOM());
      else // if it's NOT a beacon launch...
	binning.push_back(Binning(om, start, start + n*width, n));
    }
  }
  return binning;
