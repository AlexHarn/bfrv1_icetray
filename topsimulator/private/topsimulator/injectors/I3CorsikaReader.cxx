...
bool I3CorsikaReader::NextSubBlock()
{
    ...
    case CorsikaSubBlock::EVTE:
    {
	  ...
	  // In the following, subBlock_[7] is reinterpreted as an int32, increased by one, then interpreted back as a float and compared to numParticles.
	  CORSIKA_WORD n = subBlock_.GetWord(7);
	  n.INT32 += 1;
	  int64_t error = static_cast<int64_t>(n.FLOAT) - numParticles;
	  // therefore, the following checks if particleCounter_ is within (numParticles-1, numParticles+1)
