...
namespace {
  ...
  const double DIST_BIN_WIDTH = 10.0*I3Units::cm;

  // Maximum zenith angle for weighted particles
  const double MAX_ZENITH = 88.9*I3Units::deg;

  TankKey
  GetTankKey(std::string key)
  {
    TankKey::TankID tank;
  ...
  {
    DensityEntry_t(): count(0), value(0.0) {}
    unsigned int count;
    double value;
  };
  ...
  double GetOptDistance(const DensityMap_t& dMap, const double r0)
  {
    // This method determines the optimal resampling radius for a tank
    // Total number of particles within r0 from tank
    ...
    for(density_iter = dMap.begin(); density_iter!=dMap.end(); ++density_iter) {
      ...
      double diff2 = fabs(partW - counter*(dfound*dfound)/(r0*r0));
  ...
  randomService_(I3RandomServicePtr())
{
  ...
  AddParameter("RaiseObservationLevel",
               "Adjust the z-coordinate of the observation level upward by this many meters. Only for correcting observation level problems after-the-fact. Default=0. This is overridden if CorrectObservationLevel==true",
               raiseObsLevel_);
  AddParameter("CorrectObservationLevel",
               "Automatically adjust the z-coordinate of the observation level so it is above the snow at all tanks. Default is false.",
               autoObsLevelHack_);
...
void I3CorsikaInjector::Configure()
{
  ...
  GetParameter("RandomServiceName", randomServiceName);
  ...
  {
    randomService_ = GetContext().Get<I3RandomServicePtr>(randomServiceName);
    if(randomService_) log_info("+ Random service: %s  (EXTERNAL)",  randomServiceName.c_str());
    else               log_fatal("No random service \"%s\" in context!", randomServiceName.c_str());
  }
  ...
  GetParameter("CorrectObservationLevel", autoObsLevelHack_);
...
bool I3CorsikaInjector::NextEvent(int& runID, int& evtID, I3Particle& primary, I3FrameConstPtr frame)
{
  ...
  if (frame) {
    geometry = frame->Get<I3GeometryConstPtr>("I3Geometry");
  }

  double detectorLevel = reader_.GetObsLevel();
  if (geometry && autoObsLevelHack_){
    const I3StationGeoMap& station_geo = geometry->stationgeo;
    detectorLevel = std::numeric_limits<double>::min();
    for (I3StationGeoMap::const_iterator it = station_geo.begin(); it != station_geo.end(); ++it) {
      for (unsigned int i = 0; i != it->second.size(); ++i) {
        const I3TankGeo& g = it->second.at(i);
        // g.position gives the position of the DOMs, which are on the top of the ice (fillheight below the top of the tank).
        const double z = g.position.GetZ() + 0.5*g.fillheight + g.snowheight;
        if (detectorLevel < z) detectorLevel = z;
      }
    }
  }
  else if (autoObsLevelHack_) {
    log_fatal("Geometry is not available. Observation plane can not be adjusted to be above the snow.");
  }

  if(!reader_.NextEvent(primary, currentEvtID_))
  {
    if(!reader_.NextRun(currentRunID_))            return false;
    // I3CorsikaReader::NextRun is where the z of the observation level gets computed, so
    // here is where you should check the observation level (before reading in any particles)
    const double obsLevel = reader_.GetObsLevel();
    if (autoObsLevelHack_ && obsLevel < detectorLevel)
      reader_.HackObsLevelUpwards(detectorLevel - obsLevel);
    else if (raiseObsLevel_ != 0.)
  ...
  if (weightedRandomSampling_){
    ...
    if(relocationStation_ > 0) {
      if (!geometry)
        log_fatal("Requested to place core around station %d but I3Geometry is not in the frame!", relocationStation_);
      const I3StationGeoMap::const_iterator& station_geo_it = geometry->stationgeo.find(relocationStation_);
      if(station_geo_it == geometry->stationgeo.end() || station_geo_it->second.empty())
          log_fatal("Selected station (%d) does not exist in geometry.", relocationStation_);
      const I3StationGeo& station_geo = station_geo_it->second;
      relocationX_ = 0;
      relocationY_ = 0;
      for (unsigned int tank_i = 0; tank_i != station_geo.size(); ++tank_i) {
        relocationX_ += station_geo.at(tank_i).position.GetX();
        relocationY_ += station_geo.at(tank_i).position.GetY();
      }
      relocationX_ /= station_geo.size();
      relocationY_ /= station_geo.size();
    }
...
void I3CorsikaInjector::GenerateClones(const I3Particle& particle, const double& weight)
{
  ...
  for(resp_iter=respMap->begin(); resp_iter!=respMap->end(); ++resp_iter)
  {
    ...
    {
      if (dist <= dcheck_) {
        // if dist > dcheck_, then dist > doptimal for sure and the optimum radius was not computed to begin with, so do not complain
        log_error_stream("Didn't find optimal distance for Tank " << tankKey << ". Skipping particle!");
      }
      continue;
    }
    ...
    {
      log_warn_stream("Discarding nearly horizontal (zenith = " <<
                      particle.GetZenith()/I3Units::deg << " deg) particle ("
                      << particle.GetTypeString() << ") with weight " << weight
                      << " at Tank " << tankKey << "!");
      continue;
    }
    ...
    for(int i=0; i<nclones; ++i)
    {
      ...
      z += resp_iter->second->GetZ() - 0.5*tankHeight; // the bottom of the virtual cylinder around the tank
...
void I3CorsikaInjector::ComputeOptDistances()
{
  ...
  {
    double doptimal = GetOptDistance(density_iter->second, dcheck_);
    if(!finite(doptimal))
    {
      log_error_stream("Invalid optimal distance (" << doptimal << ") of Tank "
                       << density_iter->first << ". Skipping this tank!");
      continue;
    }

    optDistMap_[density_iter->first] = doptimal;
  }
}
