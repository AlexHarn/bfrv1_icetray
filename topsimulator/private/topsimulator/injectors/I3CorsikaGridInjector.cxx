...
void I3CorsikaGridInjector::FillCenterList()
{
  ...
  if(!tankList_.empty())
  {
    ...
    for(tank_iter=tankList_.begin(); tank_iter!=tankList_.end(); ++tank_iter)
    {
      ...
      if(!response)
      {
        log_error_stream("Tank " << *tank_iter << " does not exist. Ignoring it!");
	continue;
      }
  ...
  else
  {
    ...
    for(sta_iter=stationList_.begin(); sta_iter!=stationList_.end(); ++sta_iter)
    {
      ...
      if(!tankA)
      {
        log_error_stream("Tank " << tankKeyA << " does not exist. Ignoring this station!");
	continue;
      }
      ...
      if(!tankB)
      {
        log_error_stream("Tank " << tankKeyB << " does not exist. Ignoring this station!");
	continue;
      }
