...
{
  sampleIndex_ = (sampleIndex_+1) % numSamples_;

  if (sampleIndex_ == 0) {
    // load next shower from CORSIKA file,
    // collected particle statistics on new shower
    double detectorLevel = std::numeric_limits<double>::min();
    I3TankResponseMapPtr tankResponses = responseService_->GetTankResponseMap();
    for (I3TankResponseMap::const_iterator it = tankResponses->begin(); it != tankResponses->end(); ++it) {
      // I3TankResponse::GetZ() gives the position of the _center_ of the tank
      const double z = it->second->GetZ() + 0.5 * it->second->GetTankHeight() + it->second->GetSnowHeight();
