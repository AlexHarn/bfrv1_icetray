...
class TriggerModule(HistogramModule):
    ...
    def __init__(self):
        ...
        self.append(Histogram(0, 10000, 100, "GlobalThroughputTime"))
        self.append(Histogram(0, 10000, 100, "GlobalMergedTime"))
    ...
    def DAQ(self, frame):
        ...
        for trigger in triggers:
            if trigger.key.type == dataclasses.SIMPLE_MULTIPLICITY:
                if trigger.key.source == dataclasses.ICE_TOP:
            ...
            if trigger.key.type == dataclasses.STRING:
            ...
            if trigger.key.type == dataclasses.VOLUME:
            ...
            if trigger.key.type == dataclasses.SLOW_PARTICLE:
            ...
            if trigger.key.source == dataclasses.GLOBAL:
                if trigger.key.type == dataclasses.MERGED:
                    self.histograms["GlobalMergedTime"].fill(trigger.time)
                if trigger.key.type == dataclasses.THROUGHPUT:
