...
void StartingTrackVeto::Geometry(I3FramePtr frame) {
    ...
    if(!geo_)
        log_fatal("No I3Geometry object found with name \"%s\"", geoName_.c_str());
    PushFrame(frame);
}
...
void StartingTrackVeto::DetectorStatus(I3FramePtr frame) {
    if(badDOMsName_ != "") {
        ...
        if(!badDOMsAll)
            log_fatal("No object found with name \"%s\"", badDOMsName_.c_str());
        if(!badDOMsSLC)
            log_fatal("No object found with name \"%s\"", badDOMsNameSLC.c_str());
        for(I3VectorOMKey::const_iterator i = badDOMsAll->begin(); i != badDOMsAll->end(); ++i)
            badDOMs_.push_back(*i);
        for (I3VectorOMKey::const_iterator i = badDOMsSLC->begin(); i != badDOMsSLC->end(); ++i)
            badDOMs_.push_back(*i);
    }
    PushFrame(frame);
}
...
void StartingTrackVeto::Physics(I3FramePtr frame) {
    ...
    if(!frame->Has(fitName_))
        ...
        double step_size = (timeEdgeMax_ - timeEdgeMin_) / timeEdgeNSteps_;
        for(double step = timeEdgeMin_; step <= timeEdgeMax_; step += step_size)
