...
void ObtainEventInfo(I3FramePtr frame,
    ...
    if(frame->Has("CalibrationErrata")) {
        calibrationErrata = frame->Get<I3TimeWindowSeriesMapConstPtr>("CalibrationErrata");
    ...
    for(I3OMGeoMap::const_iterator k = geo->omgeo.begin(); k != geo->omgeo.end(); ++k) {
        log_debug("-------------");
        ...
        if(omkey.GetString()>87 && omkey.GetOM()>80)
            continue; // Gen2 Compatibility

        // Check that this DOM is good
        bool badOM = false;
        for(I3VectorOMKey::const_iterator badK = badDOMs->begin(); (badK != badDOMs->end() && !badOM); ++badK)
            badOM |= ((*badK) == omkey);
        if(has_calibrationErrata)
            for(I3TimeWindowSeriesMap::const_iterator badK = calibrationErrata->begin(); (badK != calibrationErrata->end() && !badOM); ++badK)
                badOM |= (badK->first == omkey);
        log_debug("badOM=%d", badOM);
        if(badOM)
            continue;

        // Distance metric (closest approach distance to track)
        ...
        // DOM observed charge
        double qTot = 0;
        std::vector<double> ts; // pulse times
        std::vector<double> qs; // pulse charges
        for(I3RecoPulseSeriesMap::const_iterator i = (*pulses).begin(); i != (*pulses).end(); ++i) {
            if((*i).first == omkey) {
                for(const I3RecoPulse &pulse : i->second) {
        ...
        // Distance metric (closest approach distance along track from vertex)
        ...
        // Distance metric (distance of closest approach connected by Cherenkov angle to track)
        ...
        double quantileSum = 0; // sum of quantiles in all segments and time bins scaled by the expected total PE
        for(I3VectorI3Particle::const_iterator s = segments->begin(); s != segments->end(); ++s) {
            ...
            double segdat = ((*fit).GetPos() - thisPos).Magnitude(); // segment distance along track
        ...
        if(quantileSum > 0) {
            // Distance metric (average segment distance weighted by charge)
            ...
            // Previously stored value for these distance metrics, values have no error
        ...
        // Find the observed PEs which match with the expected PE (look for yield to drop below 1% of peak, thus the .01 below)
        ...
        // Insert 100 Hz of noise into every time bin
        else
...
void FindNormalization(I3FramePtr frame,
        std::string pulsesName,
        std::string fitName,
        int ls,
        double tol,
    ...
    if((!obs) || (!exp) || (!bg) || (!cherdats) || (!cads))
        ...
        log_debug("Starting");
        bool found_cdat_with_charge = false;
        double minCherdat = std::numeric_limits<double>::max();

        // Find the miniumum cherenkov distance where we observed charge,
        // without this we can't do the calculation
        // this divides the detector into two regions; light: where there is light from the muon, dark: where there is no light from the muon
        for(I3MapKeyUInt::const_iterator k = keyToIndexMap->begin(); k != keyToIndexMap->end(); ++k) {
            const OMKey& omkey = k->first;
            if(((*cherdats).find(omkey)->second <= minCherdat || !found_cdat_with_charge) && (*obs)[(*k).second] > 0) {
                found_cdat_with_charge = true;
                minCherdat = (*cherdats).find(omkey)->second;
            }
        }
        log_debug("minCherdat=%f",minCherdat);

        // Stochastics can artificially increase the yield, we only want this if they are at the beggining
        ...
        if(supress_stoch) {
            ...
            double obsAvg = obsSum/obs_cnt; // average observed charge in the light region
            ...
            // 1) If a DOM that observes light is far from the track where it is not expected this is likely due to a stochastic loss.
            //    Remove DOMs where less than .1 PE is expected but a photon is observed
            ...
            for(I3MapKeyUInt::const_iterator k = keyToIndexMap->begin(); k != keyToIndexMap->end(); ++k) {
                ...
                if((*cherdats).find(omkey)->second <= minCherdat)
                    rejectKeysCheck.push_back(omkey);
                else if((*exp)[(*k).second] <= .1 && (*obs)[(*k).second] > 0)
                    rejectKeysCheck.push_back(omkey);
                else if((*exp)[(*k).second] >= 1)
                    rejectKeysCheck.push_back(omkey);
                else if((*obs)[(*k).second] > 3 * ceil(obsAvg))
                    rejectKeysCheck.push_back(omkey);
                else
                    okKeysCheck.push_back(omkey);
            }
            ...
            if(okKeysCheck.size() > 10)
                rejectKeys = rejectKeysCheck;
        }

        // Skip if we don't have anything to calculate with
        // Otherwise build the list of keys you want to calculate with
        // TODO reorganize this block of logic to be more readable, maybe start with if(supress_stoch)
        if(found_cdat_with_charge) {
            for(I3MapKeyUInt::const_iterator k = keyToIndexMap->begin(); k != keyToIndexMap->end(); ++k) {
                const OMKey& omkey = k->first;
                bool keyOK = true;
                for(std::vector<OMKey>::const_iterator rk = rejectKeys.begin(); rk != rejectKeys.end(); ++rk) {
                    if((*rk) == omkey) {
                        keyOK = false;
                        break;
                    }
                ...
                if(cascade) {
                    // include good keys with observed charge
                    if(keyOK && (*obs)[(*k).second] > 0) {
                        // if stochastic losses are not suppressed
                        // include all OK keys in the light region
                        if(!supress_stoch && (*cherdats).find(omkey)->second > minCherdat)
                            okKeyIdxs.push_back((*k).second);
                        // if stochastics are supressed, include all OK keys with observed charge
                        else if(supress_stoch)
                            okKeyIdxs.push_back((*k).second);
                    }
                    // include bad keys if stochastics are suppressed and they are along the first 250 meters of the track
                    ...
                    if(keyOK) {
                        // if stochastic losses are not suppressed
                        // include all OK keys in the ligh region
                        if(!supress_stoch && (*cherdats).find(omkey)->second > minCherdat)
                            okKeyIdxs.push_back((*k).second);
                        // if stochastics are supressed, include all OK keys
                        else if(supress_stoch)
                            okKeyIdxs.push_back((*k).second);
                    }
                    // include bad keys if stochastics are suppressed and they are along the first 250 meters of the track
        ...
        // TODO replace this with a real minimizer
        log_debug("Normalizing");
        if(okKeyIdxs.size() > 0) {
            double obsSum = 0;
            double expSum = 0;
            log_debug("Compute exp and obs sums");
            for(std::vector<unsigned int>::const_iterator OK_idx = okKeyIdxs.begin(); OK_idx != okKeyIdxs.end(); ++OK_idx) {
        ...
        for(I3VectorDouble::const_iterator v = exp->begin(); v != exp->end(); ++v) {
...
void FindDistances(I3FramePtr frame,
        std::string pulsesName,
        std::string fitName,
        int ls) {
    std::string sls = boost::lexical_cast<std::string>(ls);
...
double AssessProbability(I3FramePtr frame,
        std::string pulsesName,
        std::string fitName,
        int ls,
        std::string dat_type,
