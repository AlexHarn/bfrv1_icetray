...
void MultiplicityStringTrigger::DAQ(I3FramePtr frame)
{
  ...
  if (fullMap == 0) {
     ...
     * Fill the StringMap
     *------------------------------------------------------------*/
    boost::shared_ptr<std::map<int,std::vector<TriggerHit>>> stringMap(new std::map<int,std::vector<TriggerHit>>);
    FillHits(fullMap, stringMap);

    /*------------------------------------------------------------*
     * Loop over the StringMap
     *------------------------------------------------------------*/
    std::map<int,std::vector<TriggerHit>>::iterator stringMapIter;
    for (stringMapIter = stringMap->begin(); 
	 stringMapIter != stringMap->end(); 
	 stringMapIter++) {
      int stringNumber = stringMapIter->first;
      auto hitVector=boost::make_shared<std::vector<TriggerHit>>(stringMapIter->second);
...
void MultiplicityStringTrigger::FillHits(I3DOMLaunchSeriesMapConstPtr fullMap, 
					 boost::shared_ptr<std::map<int,std::vector<TriggerHit>>> stringMap)
{
