...
namespace DetectorStatusUtils{
  ...
  BOOST_PARAMETER_FUNCTION(
	...
	return std::vector<TriggerKey>();
      ...
      std::stringstream error_str; // only used for reporting errors
	...
	error_str << vtype.first << std::endl;
	
      }

      if(key_matches.size() == 0){
	// no match was found
	log_error("Here's a list of all keys from the Detector Status: \n%s", error_str.str().c_str());
      }
  ...
  BOOST_PARAMETER_FUNCTION(
	...
	if(key_matches.size() == 0 ){
	  ...
	  log_error("Can't resolve ambiguity.");
	  log_error("Here are possible matches: \n%s",
		    sstr.str().c_str());
	}
