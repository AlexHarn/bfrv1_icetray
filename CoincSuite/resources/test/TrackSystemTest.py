...
"""
UNITTEST for TrackSystemTester: Create a artificial series of testframes and check if the processing goes through
"""
  ...
  def testSequence(self):
    if (self.frame.Stop == icetray.I3Frame.Physics): #just to be sure
      if (self.pframes == 0):
        ...
        print(rs.keys())
        self.assert_(rs.keys()==["HypoTrackSystemTester","MutualTrackSystemTester"])
        self.assert_(rs["HypoTrackSystemTester"]==True)
        self.assert_(rs["MutualTrackSystemTester"]==True)
      else:
...
tray.AddModule("TrackSystemTester", "MutualTrackSystemTester",
  SplitName = SplitName,
  HypoName = "hypoframe",
...
tray.AddModule("TrackSystemTester", "HypoTrackSystemTester",
  SplitName = SplitName,
  HypoName = "hypoframe",
  RecoMapName = SplitPulses,
  HypoFitName = "HFit",
