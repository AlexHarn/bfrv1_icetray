...
"""
UNITTEST for cogCausalConnectTester: Create a artificial series of testframes and check if the processing goes through
"""
...
class ScenarioSource(icetray.I3Module):
  ...
  def Process(self):
    ...
    recopulse2.time = 1000./I3Constants.c #= 3333,3... ns
    recopulse2.charge = 2.
...
class TestDaqSequence(unittest.TestCase):
  ...
  def testSequence(self):
    if (self.frame.Stop == icetray.I3Frame.DAQ):
      ...
      self.assert_(ra[0] == "cogCausalConnectTester")
  ...
  def testSequence(self):
    if (self.frame.Stop == icetray.I3Frame.Physics): #just to be sure
      if (self.pframes == 0):
        ...
        self.assert_(rs.keys()==["cogCausalConnectTester"])
        self.assert_(rs["cogCausalConnectTester"]==True)
      else:
...
tray.AddModule("cogCausalConnectTester", "cogCausalConnectTester", #values choosen as small as possible, whith small margin
  SplitName = SplitName,
  HypoName = "hypoframe",
  RecoMapName = SplitPulses,
  HypoFitName = "HFit",
  TravelTimeResidual = dataclasses.make_pair(-20.*I3Units.ns, 20.*I3Units.ns),
  WallTime = 1000./I3Constants.c,
  MaxVerticalDist = 1005.*I3Units.m,
  MaxHorizontalDist = 0.*I3Units.m,
  MaxTrackDist = 10.*I3Units.m,
  MaxFurthestDist = 150.*I3Units.m)
