...
"""
UNITTEST for CylinderPulsesTester: Create a artificial series of testframes and check if the processing goes through
"""
...
class ScenarioSource(icetray.I3Module):
  ...
  def Process(self):
    ...
    P2fit.dir= dataclasses.I3Direction(180.*I3Units.deg, 0.) #straight up
    ...
    Hfit.time = 0.5
    Hfit.pos=geo.omgeo[icetray.OMKey(36,60)].position
...
class TestDaqSequence(unittest.TestCase):
  ...
  def testSequence(self):
    if (self.frame.Stop == icetray.I3Frame.DAQ):
      ...
      self.assert_(ra[0] == "MutualCylinderPulsesTester")
      self.assert_(ra[1] == "HypoCylinderPulsesTester")
  ...
  def testSequence(self):
    ...
    if (self.frame.Stop == icetray.I3Frame.Physics): #just to be sure
      if (pframes == 0):
        ...
        self.assert_(rs.keys()==["HypoCylinderPulsesTester", "MutualCylinderPulsesTester"])
        self.assert_(rs["HypoCylinderPulsesTester"]==True)
        self.assert_(rs["MutualCylinderPulsesTester"]==True)
      else:
...
#icetray.logging.set_level(icetray.I3LogLevel.LOG_DEBUG)

tray.AddModule(ScenarioSource, "ScenarioSource",
                GCDfile = GCDfile)

tray.AddModule("CylinderPulsesTester", "MutualCylinderPulsesTester",
  SplitName = SplitName,
  HypoName = "hypoframe",
...
from icecube import CoincSuite
tray.AddModule("CylinderPulsesTester", "HypoCylinderPulsesTester",
  SplitName = SplitName,
  HypoName = "hypoframe",
  RecoMapName = SplitPulses,
  HypoFitName = "HFit",
