...
I3MCPE makeHit(double time, unsigned int weight=1, I3ParticleID parentID={0,-1}){
	I3MCPE hit(parentID.majorID,parentID.minorID);
	hit.time=time;
	hit.npe=weight;
	return(hit);
}
...
public:
	...
	param_setter<PMTResponseSimulatorTestSetup> Configure(){
		return(param_setter<PMTResponseSimulatorTestSetup>(*this,r->configuration_));
    }
	void FinalizeConfiguration(){
		r->Configure_();
	}
...
bool isTimeOrdered(const std::vector<I3MCPulse>& hits){
	return(std::is_sorted(hits.begin(),hits.end(),
	  [](const I3MCPulse& p1, const I3MCPulse& p2){ return(p1.time<p2.time); }));
}
