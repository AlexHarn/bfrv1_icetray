...
//======================================
// Helper functions linked from CombinedSimulatorTests.cxx

//function for quickly building simple hits
I3MCPE makeHit(double time, unsigned int weight=1, I3ParticleID parentID={0,-1});
...
class CombinedSimulatorTestSetup{
private:
...
protected:
	I3Context& context;
	boost::shared_ptr<CombinedSimulator> r;
public:
	CombinedSimulatorTestSetup(I3Context& context):
	inbox(new std::deque<boost::shared_ptr<I3Frame>>),
	collector(new CollectorModule(context)),
	context(context),
	r(new CombinedSimulator(context))
	{
#ifdef __clang__
...
#endif
		r->inbox_=inbox;
		r->ConnectOutBox("OutBox",collector);
	}
	
	param_setter<CombinedSimulatorTestSetup> Configure(){
		return(param_setter<CombinedSimulatorTestSetup>(*this,r->configuration_));
    }
	void FinalizeConfiguration(){
		r->Configure_();
	}
	...
	CombinedSimulator& operator*() const{ return(*r); }
	CombinedSimulator* operator->() const{ return(r.get()); }
};
...
boost::shared_ptr<I3Frame> frameWithGeometry(){
	I3Map<OMKey, I3OMGeo> omgeo;
	omgeo[testOM1] = I3OMGeo();
	omgeo[testOM2] = I3OMGeo();
	omgeo[testOM3] = I3OMGeo();
	omgeo[testOM4] = I3OMGeo();

	omgeo[testOM1].omtype = I3OMGeo::IceCube;
	omgeo[testOM2].omtype = I3OMGeo::IceCube;
	omgeo[testOM3].omtype = I3OMGeo::IceCube;
	omgeo[testOM4].omtype = I3OMGeo::IceCube;

	boost::shared_ptr<I3Geometry> geometry = boost::make_shared<I3Geometry>();
	geometry->omgeo = omgeo;
	boost::shared_ptr<I3Frame> frame(new I3Frame(I3Frame::Geometry));
	frame->Put("I3Geometry", geometry);

	return(frame);
}

I3DOMCalibration makeDOMCalibration()
{
	I3DOMCalibration DOMcal;
	
	LinearFit pmtCalibFit;
	pmtCalibFit.slope = 1;
	pmtCalibFit.intercept = 1;
	DOMcal.SetPMTDiscCalib(pmtCalibFit);

	LinearFit transitTimeFit;
	transitTimeFit.slope = 1;
	transitTimeFit.intercept = 1;
	DOMcal.SetTransitTime(transitTimeFit);

	LinearFit HVGainFit;
	HVGainFit.slope = 7.5;
	HVGainFit.intercept = 16.5;
	DOMcal.SetHVGainFit(HVGainFit);

	TauParam tauparameters;
	tauparameters.P0 = 1.;
	tauparameters.P1 = 1.;
	tauparameters.P2 = 1.;
	tauparameters.P3 = 1.;
	tauparameters.P4 = 1.;
	tauparameters.P5 = 1.;
	tauparameters.TauFrac = 1.;
	DOMcal.SetTauParameters(tauparameters);

	DOMcal.SetFrontEndImpedance(1);

	DOMcal.SetTemperature(220);

	return DOMcal;
}

boost::shared_ptr<I3Frame> frameWithCalibration(){
		
	std::map<OMKey, I3DOMCalibration> domCal;
	domCal[testOM1] = makeDOMCalibration();
	domCal[testOM2] = makeDOMCalibration();
	domCal[testOM3] = makeDOMCalibration();
	domCal[testOM4] = makeDOMCalibration();
	
	boost::shared_ptr<I3Calibration> calibration = boost::make_shared<I3Calibration>();
	calibration->domCal = domCal;
	boost::shared_ptr<I3Frame> frame(new I3Frame(I3Frame::Calibration));
	frame->Put("I3Calibration", calibration);

	return(frame);
}

boost::shared_ptr<I3Frame> frameWithDetectorStatus(){
	
	std::map<OMKey, I3DOMStatus> domStatus_;
	domStatus_[testOM1] = I3DOMStatus();
	domStatus_[testOM2] = I3DOMStatus();
	domStatus_[testOM3] = I3DOMStatus();
	domStatus_[testOM4] = I3DOMStatus();
	domStatus_[testOM1].pmtHV = 1.;
	domStatus_[testOM2].pmtHV = 1.;
	domStatus_[testOM3].pmtHV = 1.;
	domStatus_[testOM4].pmtHV = 1.;
	domStatus_[testOM1].speThreshold = 1.5;
	domStatus_[testOM2].speThreshold = 1.5;
	domStatus_[testOM3].speThreshold = 1.5;
	domStatus_[testOM4].speThreshold = 1.5;
	domStatus_[testOM1].lcSpan = 1;
	domStatus_[testOM2].lcSpan = 1;
	domStatus_[testOM3].lcSpan = 1;
	domStatus_[testOM4].lcSpan = 1;
		
	boost::shared_ptr<I3DetectorStatus> detectorStatus = boost::make_shared<I3DetectorStatus>();
	detectorStatus->domStatus = domStatus_;
	boost::shared_ptr<I3Frame> frame(new I3Frame(I3Frame::DetectorStatus));
	frame->Put("I3DetectorStatus", detectorStatus);

	return(frame);
}

//function for quickly building simple hits
I3MCPE makeHit(double time, unsigned int weight, I3ParticleID parentID);

//give the test OM a typical gain as a function of high voltage
boost::shared_ptr<I3Calibration> getTestCalibration2(){
	...
	if(!testCalibration){
		...
		testCalibration->domCal[testOM1].SetHVGainFit(hvGainCal);
		testCalibration->domCal[testOM2].SetHVGainFit(hvGainCal);
		testCalibration->domCal[testOM3].SetHVGainFit(hvGainCal);
		testCalibration->domCal[testOM4].SetHVGainFit(hvGainCal);
	}
	return(testCalibration);
}

//set the test OM to the voltage necessary for a gain of 10^7
boost::shared_ptr<I3DetectorStatus> getTestStatus2(){
	static boost::shared_ptr<I3DetectorStatus> testStatus;
	if(!testStatus){
		testStatus=boost::make_shared<I3DetectorStatus>();
		testStatus->domStatus[testOM1].pmtHV=1.359e3*I3Units::V;
		testStatus->domStatus[testOM2].pmtHV=1.359e3*I3Units::V;
		testStatus->domStatus[testOM3].pmtHV=1.359e3*I3Units::V;
		testStatus->domStatus[testOM4].pmtHV=1.359e3*I3Units::V;
	}
	return(testStatus);
}

//put a set of hits, assigned to the test OMs, into a frame along with necessary calibration and status data
boost::shared_ptr<I3Frame> frameWithData2(const std::vector<I3MCPE>& hits){
	boost::shared_ptr<I3Frame> frame(new I3Frame(I3Frame::DAQ));
	frame->Put(getTestCalibration2());
	frame->Put(getTestStatus2());
...
TEST(1_JitterDistribution){
	...
	rng.installService();
	CombinedSimulatorTestSetup rts(context);
	rts.Configure()("UsePMTJitter",true)("PrePulseProbability",0)("LatePulseProbability",0)("AfterPulseProbability",0)("ApplySaturation",false)("MergeHits",false);
	
	boost::shared_ptr<I3Frame> geometry = frameWithGeometry();
	rts.processFrame(geometry);

	boost::shared_ptr<I3Frame> calibration = frameWithCalibration();
	rts.processFrame(calibration);

	boost::shared_ptr<I3Frame> detectorStatus = frameWithDetectorStatus();
	rts.processFrame(detectorStatus);

	boost::shared_ptr<I3Frame> frame=frameWithData2(hits);
	frame=rts.processFrame(frame);
	
	ENSURE_DISTANCE(1,1,.02,"It just did not crash");
}
...
TEST(1b_JitterDistributionWithMerging){
	...
	rng.installService();
	CombinedSimulatorTestSetup rts(context);
	rts.Configure()("UsePMTJitter",true)("PrePulseProbability",0)("LatePulseProbability",0)("AfterPulseProbability",0)("ApplySaturation",false)("MergeHits",true);
	...
	for(const I3MCPulse& pulse : outputHits.at(testOM))
		mv.Add(pulse.time, pulse.charge);
...
TEST(2_TimeOrdering){
	...
	I3Context context;
	ServiceFactoryWrapper rng(context,"I3GSLRandomServiceFactory");
	rng.Configure()("Seed",67584);
	rng.installService();
	CombinedSimulatorTestSetup rts(context);
	rts.Configure()("UsePMTJitter",false)("PrePulseProbability",0)("LatePulseProbability",0)("AfterPulseProbability",0)("ApplySaturation",false)("MergeHits",false);
...
TEST(2b_TimeOrdering_realistic){
	...
	rts.Configure()("UsePMTJitter",true)("PrePulseProbability",.1)("LatePulseProbability",.1)("AfterPulseProbability",.1)("ApplySaturation",false)("MergeHits",false);
...
TEST(6_SPEPeak){
	...
	double histogram[chargeBins]={0};
	
	for(const I3MCPulse& pulse : outputHits.at(testOM)){
		const double q=pulse.charge;
		const unsigned int bin=(q-baseCharge)/chargeStep;
...
TEST(7_Saturation){
	...
	rng.installService();
	CombinedSimulatorTestSetup rts(context);
	rts.Configure()("UsePMTJitter",true)("PrePulseProbability",0)("LatePulseProbability",0)("AfterPulseProbability",0)("ApplySaturation",true)("UseSPEDistribution",false);
...
T acculumateHitPropertyForType(const I3Map<OMKey,std::vector<I3MCPulse> >& hits, T(I3MCHit::*propertyGetter)() const,I3MCPulse::PulseSource chosenType){
	T sum=0;
	for(const std::pair<OMKey,std::vector<I3MCPulse>>& om_entry : hits){
		for(const I3MCPulse& hit : om_entry.second){
...
TEST(8_WholeShebang){
	...
	const double expectedAverageRegularCharge=.69;
	...
	ENSURE_DISTANCE(totalEarlyAfterCharge/totalEarlyAfterHits,expectedAverageEarlyAfterCharge,.1,"Expected average early afterpulse charge");
}

TEST(IngestMergedPE){
	std::vector<I3ParticleID> particles;
	for(unsigned int i=0; i<5; i++)
		particles.emplace_back(992,i);
	std::vector<I3MCPE> hits;
	ParticlePulseIndexMap parents;
	
	//hit 0, 3 parents={0,1,2}
	hits.push_back(makeHit(0.,3));
	parents[particles[0]].push_back(0);
	parents[particles[1]].push_back(0);
	parents[particles[2]].push_back(0);
	
	//hit 1, 3 parents={2,3,4}
	hits.push_back(makeHit(1.,6)); //weight > number of parents!
	parents[particles[2]].push_back(1);
	parents[particles[3]].push_back(1);
	parents[particles[4]].push_back(1);
	
	//hit 2, 0 parents=noise
	hits.push_back(makeHit(2.,1));
	
	//hit 3, 1 parent={2}
	hits.push_back(makeHit(3.,2)); //2 counts!
	parents[particles[2]].push_back(3);
	
	//hit 4, 0 parents=noise
	hits.push_back(makeHit(4.,2)); //2 counts!
	
	I3Context context;
	ServiceFactoryWrapper rng(context,"I3GSLRandomServiceFactory");
	rng.Configure()("Seed",37562);
	rng.installService();
	CombinedSimulatorTestSetup rts(context);
	rts.Configure()("UsePMTJitter",false)
    ("PrePulseProbability",0)
    ("LatePulseProbability",0)
	...
	boost::shared_ptr<I3Frame> frame=frameWithData(hits);
	{
		boost::shared_ptr<I3ParticleIDMap> allParents(new I3ParticleIDMap);
		(*allParents)[testOM]=parents;
		frame->Put("I3MCPESeriesMapParticleIDMap",allParents);
	}
	frame=rts.processFrame(frame);
	...
	ENSURE_EQUAL(counts[particles[0]],1);
	ENSURE_EQUAL(counts[particles[1]],1);
	ENSURE_EQUAL(counts[particles[2]],1+2+2);
	ENSURE_EQUAL(counts[particles[3]],2);
}
*/