...
unsigned int const I3DOM::digitizerBits              = 10;
///The dynamic range of the FADC and ATWD digitization in number of counts.
unsigned int const I3DOM::digitizerDynamicRange      = (1u<<I3DOM::digitizerBits)-1;// 2^10-1=1023
...
void I3DOM::DOMCalibration::Configure(const I3DOMCalibration& cal){
  ...
  //For experimental data an overall shift is applied to pulses to ensure that
  //the peak value is at exactly one. We do not simulate the incorrect charge
  ...
  if(cal.IsMeanFADCChargeValid() && cal.GetMeanFADCCharge()!=1)
...
bool I3DOM::Configure(const I3DOMCalibration& cal,
  ...
  if(beaconLaunchRate!=0){
    if(clockCycle>1/beaconLaunchRate)
      log_fatal_stream("Beacon launch rate of " << beaconLaunchRate/I3Units::megahertz
                       << " MHz is faster than the CPU clock frequency of "
                       << (1./clockCycle)/I3Units::megahertz);
    beaconLaunchPhase_ = int64_t(rng_->Uniform()/beaconLaunchRate);
    //Moving it so it matches the clock cycle phase.
    beaconLaunchPhase_ -= int64_t(beaconLaunchPhase_)%int64_t(clockCycle)-currentClockPhase_;
  }
  else
    beaconLaunchPhase_ = 0;
  return success;
}
...
bool I3DOM::AddBeaconLaunches(double startTime, double endTime,
    domlauncherutils::DCStream& dcStream){
    assert(beaconLaunchRate!=0);
	
    double t = startTime;
    //Figure out when the first beacon launch of the frame occurs
    t = (1 + int64_t(t)/int64_t(1.0/beaconLaunchRate)) * int64_t(1.0/beaconLaunchRate) + beaconLaunchPhase_;
...
void I3DOM::Launch(const DiscCross& discrx){
    ...
    if(discrx.type == HLC || discrx.type == CPU_REQUESTED){
        ...
        double ATWDDeadTime = domTime_ + nATWDBins/atwdSamplingRate_[chip] 
        + nDigitizations * ATWDReadoutDigitizeTime +
        ...
        //The DOM is busy during the HLC readout + 2 clock cycles
        //however one of the clock cycles is already accounted for 
        //in the discriminator so only one clock cycle is added 
        busyTo_ = discrx.time + HLCReadoutTime + clockCycle;
