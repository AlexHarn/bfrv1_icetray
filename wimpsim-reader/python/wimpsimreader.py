...
from icecube.icetray.i3logging import log_notice
...
def WimpSimReaderEarth(tray, name, GCDFileName=None, Infile=None, StartMJD=None, EndMJD=None):
  ...
  if GCDFileName is not None:
    log_notice("Using (GCDFile)DetectorStatus.start_time for timing, in lieu of StartMJD and EndMJD.")
    time = get_time(dataio.I3File(GCDFileName))
    StartMJD = time
    EndMJD = time
