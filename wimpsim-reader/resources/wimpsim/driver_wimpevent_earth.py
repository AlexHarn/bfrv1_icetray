...
if options.FILEINDEX==0:
    infile_name = 'wa-m%d-ch%d-earth-%06d.dat' % (options.MASS, options.CHANNEL, options.RUN)
    outfile_name = 'we-m%d-ch%d-earth-%06d.dat' % (options.MASS, options.CHANNEL, options.RUN)
    final_name = 'we-m%d-ch%d-earth.%06d.dat.gz' % (options.MASS, options.CHANNEL, options.RUN)
else:
...
pathes.outdir = (options.OUTPUTDIR+'/Earth') #'/data/user/grenzi/data/EarthWimpData/Earth'
...
if os.access(pathes.outdir,os.W_OK) == False:
