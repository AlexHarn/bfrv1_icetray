...
.. note::
    ...
    The current version of the code only calculates a value for at least 3 hits,
    in every other case, NaN is returned.
...
The TrackHitsDistributionSmoothness is the maximum relative deviation of the pulses 
from a unifrom distribution along the track.

Be :math:`l_i` the length of the track to the :math:`i` th pulse in regard to the first pulse in the sorted distance list:

.. math::

    TrackHitsDistributionSmoothness := \\frac{j}{n} - \\frac{l_j}{l_n}

with :math:`j` being the pulse with:

.. math::

    max( \\left| \\frac{j}{n} - \\frac{l_j}{l_n} \\right| )

The first and the last (:math:`n` th) pulse are ignored.


TrackHitsSeparationLength
^^^^^^^^^^^^^^^^^^^^^^^^^

Calculates the COG of the first quarter of hits (hits are sorted by time) and the COG of the last quarter of hits.
TrackHitsSeparationLength is then the distance between the projection of the two COGs onto the LineFit.
