...
parser.add_option("-b", "--i3BuildDir", default="/home/vemcal/combo/build", dest="I3D", help="IceTray build directory")
...
for year in yearList:
    tempList = glob.glob("%s/%d/filtered/PFFilt/[0-9][0-9][0-9][0-9]/PFFilt_PhysicsFiltering_Run*_Subrun00000000_00000000.tar.bz2" % (inBaseDir, year))
    tempList.sort()
...
for file in fileList:
    ...
    if(len(glob.glob(("*_Run%08d*.root" % runID)))==0):
        ...
        f.write("executable   = /home/vemcal/combo/src/vemcal/resources/scripts/vemcal_process.py\n")
        f.write("arguments    = --runID=%d --inBaseDir=%s --outDir=%s\n" % (runID, inBaseDir, outDir))
        f.write("getenv       = True\n")
        f.write("log          = /home/vemcal/log/IceTop_VEMCalData_Run%08d.log\n" % runID)
