...
def OnlineCalibration(tray, name, simulation = False,
	...
	tray.AddModule('I3TopSLCPulseExtractor', 'tpx_slc',
		       ...
		       VEMPulses='', #SLC pulses are not calibrated, so do not fill the VEM pulses
           BadDomList=filter_globals.IceTopBadDoms)

	# Combine IceTop HLC and SLC, ready for sDST
	# WRONG COMMENT: Combine the InIce and IceTop RecoPulseSeries into a single RPS, ready for DST
